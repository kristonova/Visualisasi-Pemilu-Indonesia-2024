...
- **Kontes yang Tersedia:** Dataset 2024 memuat tiga kontes, yaitu `pilpres`, `dpr`, dan `dprdprov`, yang berbagi satu hierarki wilayah dan dibangun dalam satu proses *build*. Kontes `dprdkab` belum tersedia karena sumber penarikannya belum dihimpun. Surat suara DPR RI hanya memuat 18 partai nasional (`partai-1` s.d. `partai-17` dan `partai-24`); nomor urut 18–23 adalah partai lokal Aceh yang hanya berkompetisi pada pemilihan DPRA dan DPRK. Sebaliknya, kontes `dprdprov` memakai seluruh 24 kolom karena surat suara DPRA di Aceh memang memuat keenam partai lokal tersebut, sehingga nilai nol pada kolom 18–23 di luar Aceh berarti partainya tidak tercetak pada surat suara.
- **Cakupan Data Sirekap:** Data hasil 2024 diperoleh dari penarikan sistem Sirekap KPU. Karena publikasi konversi angka Sirekap sempat dihentikan oleh KPU, pada Pilpres tercatat 645.836 dari total 823.378 TPS yang memiliki data numerik, pada DPR RI 426.926 dari 823.378 TPS, dan pada DPRD Provinsi 378.445 dari 823.236 TPS. Sisanya hanya menyediakan pindaian formulir Model C.Hasil (C1-Plano).
